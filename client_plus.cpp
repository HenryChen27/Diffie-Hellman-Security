...
bool operator!=(HEXnum x,HEXnum y){		//重载大数不等于 
	return !(x==y);
}
...
HEXnum operator/(HEXnum x,HEXnum y){		//重载大数除 
	
	HEXnum ret;
	for(int i=keyLen-1;i>=0;i--) ret.num[i]=0;
	if(y>x) return ret;
	int q=0;
	while(x>=(y<<q)&&(y<<q).num[keyLen-1]==0) q++;
	q--;
	while(q>=0){
		if(x>=(y<<q))
		{
...
HEXnum hpow(HEXnum x,HEXnum y,HEXnum pp){			//大数快速幂 
	HEXnum ret=toHEX(1);	
	while(y>toHEX(0)){
		if(y.num[0]) ret=ret*x%pp;
		x=x*x%pp;
		y=y>>1;
	}
	return ret;
...
bool isPrime(HEXnum x){		//判断是否为素数 
	for(int i=1;i<=5;i++){
		HEXnum a=randHEX(toHEX(2),x-toHEX(1));
		HEXnum b=hpow(a,x-toHEX(1),x);
		if(b==toHEX(1)) continue;
		else return 0;	
...
HEXpair HEXmod(HEXnum x,HEXnum y,HEXnum pp){		//计算乘法逆元 
	
	if(y>x) return HEXmod(y,x,pp);
	HEXnum t=x/y,z=x%y;
	if(z==toHEX(1))
	{
		HEXpair ret;
	...
	HEXpair bef=HEXmod(y,z,pp),ret;
	
	ret.fst=bef.scd;
	ret.scd=(bef.fst+(pp-t%pp)*bef.scd)%pp;
	return ret;
}
HEXnum gcd(HEXnum x,HEXnum y){
	return y==toHEX(0)?x:gcd(y,x%y);
}
...
int sendHEX(int client_socket,HEXnum x,char tt){		//传送大整数 
	char ps[]={"3fac95853c508efb90f081a19662f47b5"},qs[]={"641fe7ca29a084fd5ce6c67aca8310e1"};
	HEXnum P=stringTo(ps),
	Q=stringTo(qs);
	HEXnum n=P*Q,fn=(P-toHEX(1))*(Q-toHEX(1));		//n,fn
	HEXnum e=randHEX(toHEX(2),fn-toHEX(1));
	while(gcd(e,fn)!=toHEX(1)) e=randHEX(toHEX(2),fn-toHEX(1));		//e
	HEXnum d=HEXmod(e,fn,fn).scd;		//d
	HEXnum c=hpow(x,e,n);		//密文
	char buffer[keyLen*3+1];
	printf("\n传送字符串中...\n"); 
	buffer[0]=tt;
	for(int i=0;i<keyLen;i++) buffer[i+1]=c.num[i]+'0';
	for(int i=0;i<keyLen;i++) buffer[keyLen+i+1]=n.num[i]+'0';
	for(int i=0;i<keyLen;i++) buffer[keyLen*2+i+1]=d.num[i]+'0';
	buffer[keyLen*3+1]='\0';
	send(client_socket,buffer,keyLen*3+1,0);
	usleep(200000);
	return 1;
}
HEXnum recvHEX(int client_socket){		//接收大整数 
	...
	char buffer[keyLen*3+1];
	int r=recv(client_socket,buffer,keyLen*3+1,0);
	if(r<=0) printf("error!!\n");
	HEXnum c,n,d;
	printf("\n接收字符串中...\n"); 
	for(int i=0;i<keyLen;i++) c.num[i]=buffer[i+1]-'0';
	for(int i=0;i<keyLen;i++) n.num[i]=buffer[i+keyLen+1]-'0';
	for(int i=0;i<keyLen;i++) d.num[i]=buffer[i+keyLen*2+1]-'0';
	HEXnum ret= hpow(c,d,n);
	return ret;
}
int Sbox[16][16] = {
...
void clientAffair(int client_socket){		//客户端事务 
//	while(1){		//周期性更新密钥 
		//Diffie-Hellman协议 
		char prime[65]={"FFFFFFFF00000001000000000000000000000000FFFFFFFFFFFFFFFFFFFFFFFF"};
		p=stringTo(prime);
		HEXnum g=toHEX(2+(rand()&1)*3);
		HEXnum x=randHEX(toHEX(2),p-toHEX(2)),X=hpow(g,x);
		sendHEX(client_socket,g,'g');
		sendHEX(client_socket,p,'p');
		sendHEX(client_socket,X,'X');
		
		HEXnum Y=recvHEX(client_socket);
		HEXnum k1=hpow(Y,x),k2;
		sendHEX(client_socket,k1,'K');
		k2=recvHEX(client_socket);
			
		if(k1==k2) printf("\n对称密钥正确,身份验证通过!请输入传输内容：\n");
		else
		{
		...
		for(int i=1;i<=5;i++){
			...
			prtHEX(k1); 
			putchar('\n');
			string m=encoding(s,k1);
			cout<<"密文：\n"<<m<<endl;
			char buffer[4096];
			for(int i=0;i<=m.length();i++)  buffer[i]=m[i];
			send(client_socket,buffer,m.length(),0);
...
void toString(HEXnum x){		//以字符串方式输出大数
	int tmp=0;
	for(int i=0;i<keyLen;i++){
		tmp=(tmp<<1)+x.num[i];
		if(i%8==7) {
...
HEXnum stringToHEX(char *s){		//将字符串转化为大数
	HEXnum ret;
	int len=strlen(s);
	for(int i=0;i<keyLen;i++) ret.num[i]=0;
	for(int i=0;i<len;i++)
		for(int k=0;k<8;k++)
			ret.num[i*8+k]=(s[i]&(1<<(7-k)))>0;
...
{ 
	...
	target.sin_addr.s_addr = inet_addr("192.168.72.130");// 目标IP地址
